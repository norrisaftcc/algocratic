// INFRARED Clearance Guide Scripts
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setInterval(() => {
        ...
        // Keep within 5-25% range (very low for INFRARED)
        newWidth = Math.max(5, Math.min(25, newWidth));
        loyaltyBar.style.width = newWidth + '%';
        
        // Update status text
        if (newWidth < 10) {
            loyaltyStatus.textContent = 'CRITICAL';
            loyaltyBar.style.backgroundColor = '#ff0000';
        } else if (newWidth < 20) {
            loyaltyStatus.textContent = 'PROBATIONARY';
            loyaltyBar.style.backgroundColor = '#8b0000';
        } else {
    ...
    console.log('INFRARED clearance page access logged. Timestamp: ' + new Date().toISOString());
    ...
    setInterval(() => {
        ...
        console.log('%c' + surveillanceMessages[randomIndex], 'color: #8b0000');
    }, 10000);
});